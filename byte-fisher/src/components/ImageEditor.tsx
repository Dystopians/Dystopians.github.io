...
const ImageEditor: React.FC<ImageEditorProps> = ({ onClose, lang }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-3 sm:p-6">
      <div className="bg-cyber-dark border-2 border-cyber-pink w-full max-w-4xl p-4 sm:p-6 shadow-[0_0_30px_rgba(255,0,255,0.3)] flex flex-col h-[90vh]">
        
        {/* Header */}
        <div className="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6 border-b border-cyber-pink pb-2">
          <h2 className="text-2xl sm:text-3xl font-bold text-cyber-pink glitch-text">{t.imageRecon}</h2>
          <button onClick={onClose} className="self-start sm:self-auto text-cyber-pink hover:text-white font-bold text-lg sm:text-xl">[{t.close}]</button>
        </div>
        ...
        <div className="flex-1 flex flex-col md:flex-row gap-4 sm:gap-6 overflow-hidden">
          ...
          <div className="w-full md:w-1/3 flex flex-col gap-4">
            ...
            {/* Upload Area */}
            <div 
              className="border-2 border-dashed border-cyber-gray hover:border-cyber-green p-4 rounded cursor-pointer text-center transition-colors h-28 sm:h-32 flex flex-col items-center justify-center"
            ...
            >
              <input 
                ...
                className="hidden" 
              />
              <span className="text-cyber-green text-base sm:text-lg font-bold">{t.uploadSource}</span>
            ...
            <div className="flex-1 flex flex-col">
              ...
              <textarea 
                className="flex-1 bg-black/50 border border-cyber-cyan p-2 text-white resize-none focus:outline-none focus:border-cyber-pink min-h-[120px]"
            ...
            <button 
              ...
              className={`py-3 sm:py-4 font-bold text-lg sm:text-xl border-2 transition-all
